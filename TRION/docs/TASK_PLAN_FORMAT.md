...
This v0.1 format and deterministic executor are implemented and unit tested. The `inspect_fixture` W1 runner now implements two bounded arm-clearance waypoints, routes every motion batch through queued command authority and the safety shim, records observations through an explicit adapter, and commands immediate safe-state on an active abort. Its MuJoCo transport/render adapter remains open. `actuate_handle` is capped at L2 and `mate_connector` at L1 but still have execution envelopes only. Perception facts, durable mission-control upload, commit-window coupling for contact actions, full sim execution, and promotion evidence remain Phase 2 integration work.
